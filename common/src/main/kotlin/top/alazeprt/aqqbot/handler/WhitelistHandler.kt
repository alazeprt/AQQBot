...
class WhitelistHandler(val plugin: AQQBot) {
    ...
    private fun unbind(userId: String, groupId: Long, playerName: String): Boolean {
        ...
        if (!plugin.getPlayerByQQ(userId.toLong()).map { it.getName() }.toList().contains(playerName)) {
            BotProvider.getBot()?.action(SendGroupMessage(groupId, plugin.getMessageManager().get("qq.whitelist.bind_by_other", mutableMapOf(Pair("name", plugin.getPlayerByQQ(userId.toLong()).joinToString(", ")))), true))
            return false
        }
