...
class SubReset(val plugin: AQQBot): ACommand {
    override fun onCommand(command: String, sender: ASender, args: List<String>) {
        if (!sender.hasPermission("aqqbot.command.reset")) {
            sender.sendMessage(Component.text("你没有权限使用该命令!", NamedTextColor.RED))
            return
        }
        if (args.size != 4) {
            sender.sendMessage(Component.text("用法: /aqqbot whitelist reset <name/qq> <游戏名/QQ号>"))
            return
        }
        sender.sendMessage(plugin.getCommandImpl(plugin).reset(sender, args[2], args[3]))
    }
}